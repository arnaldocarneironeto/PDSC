...
public class AlunoJpaController implements Serializable
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 826103850851742857L;
	...
	public void create(Aluno aluno) throws RollbackFailureException, Exception
	{
		...
		try
		{
			utx.begin();
			em = getEntityManager();
			Escola escola_que_selecionou_este_aluno = aluno.getEscola_que_selecionou_este_aluno();
			if(escola_que_selecionou_este_aluno != null)
			{
				escola_que_selecionou_este_aluno = em.getReference(escola_que_selecionou_este_aluno.getClass(), escola_que_selecionou_este_aluno.getId());
			...
			{
				preferenciaEscolaToAttach = em.getReference(preferenciaEscolaToAttach.getClass(), preferenciaEscolaToAttach.getId());
			...
			if(escola_que_selecionou_este_aluno != null)
			{
				escola_que_selecionou_este_aluno.getAlunos_que_preferem_esta_escola().add(aluno);
				escola_que_selecionou_este_aluno = em.merge(escola_que_selecionou_este_aluno);
			}
			for(Escola preferenciaEscola: aluno.getPreferencia())
			{
				preferenciaEscola.getAlunos_que_preferem_esta_escola().add(aluno);
	...
	public void edit(Aluno aluno) throws NonexistentEntityException, RollbackFailureException, Exception
	{
		...
		try
		{
			...
			Escola escola_que_selecionou_este_alunoOld = persistentAluno.getEscola_que_selecionou_este_aluno();
			Escola escola_que_selecionou_este_alunoNew = aluno.getEscola_que_selecionou_este_aluno();
			...
			{
				escola_que_selecionou_este_alunoNew = em.getReference(escola_que_selecionou_este_alunoNew.getClass(), escola_que_selecionou_este_alunoNew.getId());
			...
			{
				preferenciaNewEscolaToAttach = em.getReference(preferenciaNewEscolaToAttach.getClass(), preferenciaNewEscolaToAttach.getId());
			...
			if(escola_que_selecionou_este_alunoOld != null && !escola_que_selecionou_este_alunoOld.equals(escola_que_selecionou_este_alunoNew))
			{
				escola_que_selecionou_este_alunoOld.getAlunos_que_preferem_esta_escola().remove(aluno);
				escola_que_selecionou_este_alunoOld = em.merge(escola_que_selecionou_este_alunoOld);
			}
			if(escola_que_selecionou_este_alunoNew != null && !escola_que_selecionou_este_alunoNew.equals(escola_que_selecionou_este_alunoOld))
			{
				escola_que_selecionou_este_alunoNew.getAlunos_que_preferem_esta_escola().add(aluno);
				escola_que_selecionou_este_alunoNew = em.merge(escola_que_selecionou_este_alunoNew);
			}
			for(Escola preferenciaOldEscola: preferenciaOld)
			{
				if(!preferenciaNew.contains(preferenciaOldEscola))
				{
					preferenciaOldEscola.getAlunos_que_preferem_esta_escola().remove(aluno);
			...
			for(Escola preferenciaNewEscola: preferenciaNew)
			{
				...
				{
					preferenciaNewEscola.getAlunos_que_preferem_esta_escola().add(aluno);
	...
	public void destroy(Integer id) throws NonexistentEntityException, RollbackFailureException, Exception
	{
		...
		try
		{
			...
			if(escola_que_selecionou_este_aluno != null)
			{
				escola_que_selecionou_este_aluno.getAlunos_que_preferem_esta_escola().remove(aluno);
				escola_que_selecionou_este_aluno = em.merge(escola_que_selecionou_este_aluno);
			}
			List<Escola> preferencia = aluno.getPreferencia();
			for(Escola preferenciaEscola: preferencia)
			{
				preferenciaEscola.getAlunos_que_preferem_esta_escola().remove(aluno);
				preferenciaEscola = em.merge(preferenciaEscola);
			}
			em.remove(aluno);
			utx.commit();
